"""Facebook Messenger & comment webhook handler."""
import logging
...
from services.facebook_service import (
    parse_messenger_events, parse_comment_events,
    send_messenger_text, verify_webhook,
)
...
def _get_or_create_conv(db: Session, sender_id: str) -> Conversation:
    conv = db.query(Conversation).filter_by(channel=Channel.facebook, external_user_id=sender_id).first()
    if not conv:
        conv = Conversation(
...
def facebook_verify(
    ...
    result = verify_webhook(
        ...
        expected_token=hub_verify_token or "",
...
async def facebook_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    for event in parse_messenger_events(payload):
        if event["text"]:
            _handle_messenger(db, event["sender_id"], event["text"])

    # Handle comment events
    for comment in parse_comment_events(payload):
        handle_comment(
            db=db, platform="facebook",
...
def _handle_messenger(db: Session, sender_id: str, text: str) -> None:
    try:
        ...
        send_messenger_text(db, sender_id, reply)

    except Exception as e:
        logger.error("Facebook Messenger handler error for %s: %s", sender_id, e)
